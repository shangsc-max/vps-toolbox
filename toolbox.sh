...
function show_help() {
    ...
    echo -e "${BLUE}==================================================${NC}"
    echo -e "1. 快捷调调取：输入 ${YELLOW}sm${NC} 或 ${YELLOW}SM${NC} 即可启动。"
    echo -e "2. 逻辑说明：子菜单输入 ${YELLOW}0${NC} 返回上一级，主菜单输入 ${YELLOW}q${NC} 退出。"
    echo -e "3. Docker加固：解决 Docker 自动修改防火墙规则的漏洞。"
    echo -e "${BLUE}==================================================${NC}"
    read -p "按回车返回主菜单..."
}
    ...
    while true; do
        ...
        case $opt in
            ...
            5) 
                ...
                systemctl restart docker; echo -e "${GREEN}加固开启${NC}"; fi; sleep 1 ;;
            6) 
                if [ -f "/etc/docker/daemon.json" ]; then rm /etc/docker/daemon.json; systemctl restart docker; echo -e "${YELLOW}已恢复默认${NC}"; fi; sleep 1 ;;
...
function manage_f2b() {
    while true; do
        ...
        read -p "选择操作: " opt
        case $opt in
            1) apt install -y fail2ban > /dev/null 2>&1; systemctl restart fail2ban; echo -e "${GREEN}完成${NC}"; sleep 1 ;;
