...
function show_help() {
    ...
    echo -e "3. Docker 加固：开启后禁止 Docker 绕过 UFW 防火墙。"
    echo -e "${BLUE}==================================================${NC}"
    read -p "按回车返回主菜单..."
}
...
function manage_ufw() {
    while true; do
        ...
        echo -e "6. ${RED}关闭 Docker 安全加固${NC}"
        echo -e "7. 查看规则列表"
        echo -e "0. 返回主菜单"
        read -p "选择操作: " opt
        case $opt in
            ...
            6) 
                [ -f "/etc/docker/daemon.json" ] && rm /etc/docker/daemon.json
                systemctl restart docker; echo -e "${YELLOW}Docker 加固已关闭${NC}"; sleep 1 ;;
            7) ufw status verbose; read -p "回车继续..." ;;
            0) break ;;
        esac
    done
}
...
while true; do
    ...
    case "$choice" in
        0) show_help ;;
        1) read -p "GitHub 用户: " gu; wget -qO- https://github.com/$gu.keys >> ~/.ssh/authorized_keys; chmod 600 ~/.ssh/authorized_keys; echo -e "${GREEN}密钥同步完成${NC}"; sleep 1 ;;
        2) manage_ssh ;;
        3) manage_ufw ;;
        4) manage_f2b ;;
